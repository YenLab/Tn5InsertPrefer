...
#5. Filter bam reads using [samtools]
...
do
	...
	ILLUMINACLIP:${trimmomatic_path}/adapters/NexteraPE-PE.fa:2:30:10:1:true MINLEN:36  
	...
	-x ${BOWTIE2_INDEXES} -1 ${FILE_PREFIX}_R1_trimmed.fastq.gz -2 ${FILE_PREFIX}_R2_trimmed.fastq.gz > ${FILE_PREFIX}.sam
	...
	if [[ `cat badCigars.temp | wc -l` -gt 0 ]]; then
		cat ${FILE_PREFIX}.sam \
		| grep -v -F -f badCigars.temp \
		| samtools view -@ ${RUN_THREASHOLD} -Sb - \
		| samtools sort -@ ${RUN_THREASHOLD} - > ${FILE_PREFIX}.bam
	else
		samtools view -@ ${RUN_THREASHOLD} -Sb ${FILE_PREFIX}.sam \
		| samtools sort -@ ${RUN_THREASHOLD} - > ${FILE_PREFIX}.bam
	fi
	...
	# ============= 
	#This part is for generating Tn5 cut sites with duplicates remained
	dup_cutsites="True"
	if [[ ${dup_cutsites} == "True" ]]; then
