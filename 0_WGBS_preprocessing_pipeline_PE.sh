#!/bin/bash
##########################################################################
#This script takes a directory of Pair-end fastq WGBS reads, 
...
do
	...
	# =============
	# Map with BisMark2
	bismark -p 8 --multicore 1 --non_directional --nucleotide_coverage --output_dir ./ --genome ${sp_dir} \
	-1 ${FILE_PREFIX}_R1_trimmed.fastq.gz -2 ${FILE_PREFIX}_R2_trimmed.fastq.gz
	
	deduplicate_bismark --bam --paired --output_dir ./ ${FILE_PREFIX}_R1_trimmed_bismark_bt2_pe.bam

	# --parallel 10 is likely to use around 30 cores of system resources.
	bismark_methylation_extractor --paired-end --no_overlap --gzip --multicore 10 --buffer_size 30G --ignore 10 --ignore_r2 10 \
	--cytosine_report --output ./ --genome_folder ${sp_dir} ${FILE_PREFIX}_R1_trimmed_bismark_bt2_pe.deduplicated.bam
	
	#bam2nuc --genome_folder ${sp_dir} ${FILE_PREFIX}_R1_trimmed_bismark_bt2_pe.deduplicated.bam
