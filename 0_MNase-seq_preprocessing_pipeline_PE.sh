#!/bin/bash
##########################################################################
#This script takes a directory of Paired-end fastq MNase-seq reads.
...
do
	...
	bamCoverage --numberOfProcessors ${RUN_THREASHOLD} --bam ${FILE_PREFIX}.filtered.dedup.bam --outFileFormat bigwig --outFileName ${FILE_PREFIX}.filtered.dedup.bw \
	--binSize 1 --normalizeUsing CPM --effectiveGenomeSize ${GENOME_SIZE_NUM} --blackListFileName ${blacklist} 
